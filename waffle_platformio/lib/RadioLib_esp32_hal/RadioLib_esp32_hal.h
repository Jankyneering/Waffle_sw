#ifndef RADIOLIB_ESP32_HAL_H
#define RADIOLIB_ESP32_HAL_H
...
static uint32_t _getApbFrequency() {
...
static uint32_t _spiFrequencyToClockDiv(uint32_t freq) {
    uint32_t apb_freq = _getApbFrequency();
    if (freq >= apb_freq) {
...
class RadioLib_esp32_hal : public RadioLibHal {
public:
    // default constructor - initializes the base HAL and any needed private members
    RadioLib_esp32_hal(int8_t sck, int8_t miso, int8_t mosi)
    ...
    void spiBegin() {
        ...
        // set the clock
        this->spi->clock.val = _spiFrequencyToClockDiv(2000000);
